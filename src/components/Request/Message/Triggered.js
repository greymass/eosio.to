...
class ActionMessageTriggered extends Component {
  render() {
    ...
    if (scatterError) {
      console.log("detected", scatterError.message)
    }
      ...
      <Segment color="blue" stacked>
        <Header size="large">
          ...
          <Header.Content>
            ...
            <Header.Subheader>
              Click one of the options below to launch this transaction in your preferred wallet.
            </Header.Subheader>
          </Header.Content>
        </Header>
        <Segment>
          ...
          {(scatterResults)
            ? (
              <Segment color="green" size="large">
                ...
                <Segment>
                  <p>The transaction was successfully pushed, click the link below to view more details.</p>
                  <a
                    href={`https://${(chain !== "eos") ? `${chain.toLowerCase()}.` : ''}bloks.io/transaction/${scatterResults.transaction_id}`}
                    target="_blank"
                  >
